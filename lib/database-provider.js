...
var SQL_INSERT_SENSOR = 'INSERT INTO `sensor-reader-local` SET ?';
...
var SQL_SELECT_NOT_UPLOAD_SENSORS = [
  ...
  'FROM `sensor-reader-local` ',
  'WHERE `status` <> "UPLOADED" ',
  'LIMIT 20'
].join('\n');

var SQL_UPDATE_SENSOR = 'UPDATE `sensor-reader-local` SET status = "UPLOADED" WHERE sensor_id = ?';
var SQL_DELETE_SENSOR = 'DELETE FROM `sensor-reader-local` WHERE sensor_id = ?';
